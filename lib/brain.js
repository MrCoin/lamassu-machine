...
Brain.prototype._physicalDispense = function _physicalDispense (tx) {
  ...
  this.billDispenser.dispense(notes, function (err, result) {
    ...
    if (err) {
      // https://github.com/MrCoin/lamassu-machine/issues/3
      console.log('ERROR: There was an error with physically dispensing the bills. Scheduling the actual error to 30 seconds from now.');
      console.log(err.stack);
